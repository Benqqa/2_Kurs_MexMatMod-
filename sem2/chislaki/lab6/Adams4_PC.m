...
function [Y]=runge_Adams4(f,a,b,n,X,y0,z0)
    if(n>4)
        ...
        for i=4:length(X)-1
            ...
            Zb=Z(i)+((b-a)/n)*(-9/24*f(X(i-3),Y(i-3),Z(i-3))+37/24*f(X(i-2),Y(i-2),Z(i-2))-59/24*f(X(i-1),Y(i-1),Z(i-1))+55/24*f(X(i),Y(i),Z(i)));
            Yb=Y(i)+((b-a)/n)*(-9/24*Z(i-3)+37/24*Z(i-2)-59/24*Z(i-1)+55/24*Z(i));
            
            Y(i+1)=Y(i)+((b-a)/n)/24*(Z(i-2)-5*Z(i-1)+19*Z(i)+9*Zb);
            Z(i+1)=Z(i)+((b-a)/n)/24*(9*f(X(i+1),Yb,Zb)+19*f(X(i),Y(i),Z(i))-5*f(X(i-1),Y(i-1),Z(i-1))+f(X(i-2),Y(i-2),Z(i-2)));
        end
    else
